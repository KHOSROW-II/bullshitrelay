...
try:
    ...
    logger.info(f"TOKEN_DISCORD: {'SET' if TOKEN_DISCORD else 'MISSING'}")
    logger.info(f"TOKEN_TELEGRAM: {'SET' if TOKEN_TELEGRAM else 'MISSING'}")
    ...
    async def track_left_member(update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            ...
            logger.error(f"Error in track_left_member: {e}")
        ...
        try:
            ...
            if avatar_bytes:
                try:
                    ...
                    logger.warning(f"Failed to add avatar thumbnail: {e}")
            
            await channel.send(files=files, embed=embed)
            logger.info(f"File {filename} sent to Discord with embed")
            return True
        except Exception as e:
    ...
    async def send_text_to_discord(channel, text, user_name, avatar_bytes=None, reply_info=None):
        try:
            embed = discord.Embed(
                description=text,
                color=8305407,
            ...
            logger.info("Text message sent to Discord with embed")
        except Exception as e:
    ...
    async def telegram_message_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            ...
            avatar_bytes = None
            if user_id:
                avatar_bytes = await get_telegram_avatar_bytes(user_id, context)
            ...
            if msg and msg.text and not msg.photo and not msg.document and not msg.audio and not msg.video and not msg.voice and not msg.video_note and not msg.animation and not msg.sticker:
                await send_text_to_discord(channel, msg.text, user_name, avatar_bytes, reply_info)
                return
            ...
            if msg and (msg.photo or msg.document or msg.audio or msg.video or msg.voice or msg.video_note or msg.animation or msg.sticker):
                ...
                if file_obj:
                    ...
                    if file_bytes:
                        await send_file_to_discord(channel, file_bytes, filename, caption, user_name, platform, reply_info, avatar_bytes)
                return
        except Exception as e:
            logger.error(f"Error in telegram_message_handler: {e}")
    ...
    async def global_command(ctx, link: str = None):
        try:
            ...
            logger.error(f"Error in global_command: {e}")
    ...
    async def run_bots():
        ...
        try:
            ...
            logger.error(f"Error in run_bots: {e}")
            raise
    ...
    # اگر خطایی در سطح بالا رخ داد، لاگ کن و خارج شو
